...
$myFile = "csv/Accessory_FL Attributes.csv"; // File in which attribute option values are there.
$f = fopen($myFile, "r");
...
for($i=0;$i<count($myValue);$i++)
 ...
 $attribute_code  = $attribute_model->getIdByCode('catalog_product', 'accessory');
 $attribute       = $attribute_model->load($attribute_code);
 
if(!attributeValueExists('accessory', $myValue[$i]))
 {
	
$value['option'] = array($myValue[$i]);
if(!in_array($value['option'][0][0],$attribute_added))
{
  $option['option'][0] = $value['option'][0][0];
  $option['option'][2] = $value['option'][0][0];  
