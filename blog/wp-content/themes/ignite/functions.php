...
function tc_handle_upload_prefilter($file){
    ...
    if ($width < $height){
        return array("error"=>"Image you want to upload is not perfect square. Image height is ".$difference."px greater then image width.");
    }
    elseif ($width > $height){
        return array("error"=>"Image you want to upload is not perfect square. Image width is ".$difference."px greater then image height.");
    }
    else{
        return $file;
    } 
}
// lav butala code ended
...
function ct_ignite_featured_image() {
	...
	if ($has_image == true) {
        ...
        if(is_singular()){
            ...
            echo "<div><img style='margin:10px;' src=".$image."></div>";

            /*echo "<div class='featured-image' style=\"background-image: url('".$image."')\"></div>";*/

        } else {

           echo "<div><a title='".get_the_title()."' href='".get_permalink()."'><img style='margin:10px;' src=".$image."></a></div>";
...
// outputs the user's ed profile picture with Gravatar fallback
