 ...
 */
class Wizkunde_ConfigurableBundle_Block_Bundle_Adminhtml_Widget_Grid_Column_Renderer_Checkboxwz
    extends Mage_Adminhtml_Block_Widget_Grid_Column_Renderer_Abstract
{
    ...
    public function render(Varien_Object $row)
    {
        ...
        //
        $_class = '';
        $_p = Mage::getModel('catalog/product')->load($row->getId());
        if($_p->getTypeId() == "simple"){
            $parentIds = Mage::getModel('catalog/product_type_configurable')->getParentIdsByChild($_p->getId());
            if(isset($parentIds[0])){
                $_class = ' add_configurable_id_' . $parentIds[0];
            }
        }
        $childClass = "'.add_configurable_id_" . $row->getId() ."'";
        $onclick  = 'onclick = "checkAllChild(' . $childClass . ',this);"' ;
        return $this->_getCheckboxHtml($v, $checked,$_class,$onclick);
    }
     ...
     * @return string
     */
    protected function _getCheckboxHtml($value, $checked,$_class,$onclick)
    {
        $html = '<input type="checkbox" ';
        $html .= $onclick;
        $html .= 'name="' . $this->getColumn()->getFieldName() . '" ';
        $html .= 'value="' . $this->escapeHtml($value) . '" ';
        $html .= 'class="'. ($this->getColumn()->getInlineCss() ? $this->getColumn()->getInlineCss() : 'checkbox') . $_class .'"';
