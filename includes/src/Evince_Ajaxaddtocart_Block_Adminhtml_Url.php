...
class Evince_Ajaxaddtocart_Block_Adminhtml_Url extends Mage_Adminhtml_Block_System_Config_Form_Field {
    ...
    protected function _getHeaderHtml() {
        ...
        $output .= Mage::helper('ajaxaddtocart')->__('Selector of block to be updated such as Id / Class');
        $output .= '</th>';
        $output .= '<th style="padding: 2px; text-align: center;">';
        $output .= Mage::helper('ajaxaddtocart')->__('Updated Block name(name should be same as in XML)');
        $output .= '</th>';
        $output .= '<th>&nbsp;</th>';
        $output .= '</tr>';
        return $output;
    ...
    protected function _getAddButtonHtml() {
        return $this->getLayout()->createBlock('adminhtml/widget_button')
            ->setType('button')
            ->setClass('add')
            ->setLabel($this->__('Add More Block'))
