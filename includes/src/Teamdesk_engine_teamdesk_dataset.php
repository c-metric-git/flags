	...
	*/ 
	static public function fromXml($str)
	{
		...
		) // DataSet
		{
			...
			{
				/*$r = array();
				$xml->moveToAttribute("id"); $r["@id"] = intval($xml->value);
				$xml->moveToAttribute("modify"); $r["@m"] = $xml->value == "true";
				$xml->moveToAttribute("delete"); $r["@d"] = $xml->value == "true";
				$xml->moveToAttribute("c"); $r["@c"] = $xml->value;
				if($xml->read()) */
				$r = array();
