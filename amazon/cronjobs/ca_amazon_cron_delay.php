...
{    
    ...
    $feed_arr[$key]['isAmazonCANProduct?'] = (isset($feeds['Product - isAmazonCANProduct?']) && $feeds['Product - isAmazonCANProduct?'] != '') ? $feeds['Product - isAmazonCANProduct?'] : ''; //  canada flag
    ...
    $feed_arr[$key]['recommendedBrowseNode']        = (isset($feeds['Amazon Category - recommendedBrowseNodeCAN']) && intval($feeds['Amazon Category - recommendedBrowseNodeCAN'])>0) ? $feeds['Amazon Category - recommendedBrowseNodeCAN'] : '6741988011'; // added by lav
...

    {
        ...
        {            
            ...
            $feed_arr[$key]['variations'][$count]['recommendedBrowseNode']         = (isset($var['Amazon Category - recommendedBrowseNodeCAN']) && intval($var['Amazon Category - recommendedBrowseNodeCAN'])>0) ? $var['Amazon Category - recommendedBrowseNodeCAN'] : '6741988011'; // added by lav
            ...
            $feed_arr[$key]['variations'][$count]['isAmazonCANProduct?'] = (isset($var['Product - isAmazonCANProduct?']) && $var['Product - isAmazonCANProduct?'] != '') ? $var['Product - isAmazonCANProduct?'] : ''; // for canada            
    ...
    */
...

    {
...

        {
            ...
            $feed .= "<ProductData><".$product_feed['product_type'].">";
    ...
    $tmpfname = $_SERVER['DOCUMENT_ROOT'] .'/'.FOLDER_NAME."/cronjobs/product_xml_files/CA_product_update_".date('Y').date('m').date('d')."_".date('H').date("i").".xml";  
    ...
    if($feedVar!=""){
        ...
        $tmpfname = $_SERVER['DOCUMENT_ROOT'] .'/'.FOLDER_NAME."/cronjobs/product_varaiation_xml_files/CA_product_update_".date('Y').date('m').date('d')."_".date('H').date("i").".xml";  
            ...
            foreach($feed_arr as $key => $product_feed){
                if(isset($product_feed['variations']) && count($product_feed['variations'])>0){
                    foreach($product_feed['variations'] as $key => $product_var_feed){
                        $mysqli->query('INSERT INTO error_reporting VALUES (null, "'.$product_var_feed['product_id'].'", "'.$product_var_feed['SKU'].'","Variation Posting","'.$response->SubmitFeedResult->FeedSubmissionInfo->FeedSubmissionId.'",null,"0",null,"'.$product_var_feed['isAmazonCANProduct?'].'","")');   
                    }
                }
            }
