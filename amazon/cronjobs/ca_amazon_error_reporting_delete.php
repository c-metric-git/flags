...
$submission_list = $mysqli->query('SELECT id, product_id, sku, feed_type, feed_id FROM error_reporting WHERE processed="0" AND feed_type = "Entry Removal" AND `isAmazonCANProduct?` = "1"');
...
$dataset = $api->GetSchema("Amazon Entry",array('recordHistory','isReadyForUpdateToCanAmz?','isReadyForDeletionToCanAmz?','ASIN','amazonErrorReason','isVisibleOnAmazonCAN?'));
