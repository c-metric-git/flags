...
$dataset = $api->GetSchema("Amazon Entry",array('sku','isVisibleOnAmazonCAN?','recordHistory','amazonErrorReason','ASIN'));



$mysqli = new mysqli(DB_HOST, DB_USER, DB_PASSWORD, DB_NAME);

$mysqli->query('DELETE FROM ready_for_amazon WHERE `Product - isAmazonCANProduct?` = "1"');

$mysqli->query('DELETE FROM ready_for_amazon_child WHERE `Product - isAmazonCANProduct?` = "1"');
...
$result = $api->Query("SELECT [platinum-5],[platinum-4],[platinum-3],[platinum-2],[platinum-1],
                       ...
                       [isReadyForDeletionToCanAmz?],[isReadyForUpdateToCanAmz?],[recordHistory],[amazonErrorReason],[# of Related Amazon Entries],[Related Amazon entry],[Product - isBannedFromAmazon?],[Product - Vendor Product URL],[reasonBadMatch],[minimum-manufacturer-age-recommended-unit-of-measu],[isVisibleOnAmazonAds?],[isBadTitleOnAmz?],
                       ...
                       [imgPath],[is_visible],[isVisibleOnAmazonCAN?],[item-condition],[item-form],[item-package-quantity],[item-price],[item-specialty1],[item-specialty2],[item-specialty3],[item-specialty4],[item-specialty5],
                       ...
                       [Product - isAmazonCANProduct?],[item-priceCAN],[Amazon Category - recommendedBrowseNodeCAN]
                       
                       FROM [Amazon Entry] WHERE [Product - isAmazonCANProduct?]=true and [is_visible]=true and [isReadyForUpdateToCanAmz?]=true and [Product - flagORMD]=false and [Product - VENDOR - isBannedFromAmazon?]=false and [Product - isFlagsrusProduct?]=false ORDER BY [sku] DESC");     //[priceAmazon],[shippingCost],[shippingCostNew]
