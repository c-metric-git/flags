...
$submissionProduct_list = $mysqli->query('SELECT product_id, sku FROM error_reporting WHERE processed="0" AND (feed_type = "Product Posting" OR feed_type = "Variation Posting")  AND `isAmazonUKProduct?` = "1" GROUP BY sku');
$TDProductId = array();
...
$submission_list = $mysqli->query('SELECT id, product_id, sku, feed_type, feed_id FROM error_reporting WHERE processed="0" AND feed_type = "Product Posting"  AND `isAmazonUKProduct?` = "1" GROUP BY feed_id');
...
$dataset = $api->GetSchema("Amazon Entry",array('recordHistory','isReadyForUpdateToUkAmz?','isReadyForDeletionToUkAmz?','amazonErrorReason','isVisibleOnAmazonUK?'));
