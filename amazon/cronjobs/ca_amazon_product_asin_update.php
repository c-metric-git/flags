...
$sql = 'SELECT `@id`,`sku`,`recordHistory`,`amazonErrorReason` FROM ready_for_amazon WHERE `Product - isAmazonCANProduct?` = "1"';
...
$dataset = $api->GetSchema("Amazon Entry",array('isVisibleOnAmazonCAN?','isReadyForUpdateToCanAmz?','recordHistory','amazonErrorReason','ASIN','Amazon Title','Amazon_Style_Number'));
...

    {
            ...
            {            
                ...
                {                
...

                    {
                        ...
                        $query = $mysqli->query('UPDATE ready_for_amazon_child SET ASIN = "'.$asin.'", `recordHistory` = "'.$record_history.'", `amazonErrorReason` = "'.$amazon_error_reason.'", `isVisibleOnAmazonCAN?` = "1" WHERE `@id` = "'.$variations_val['@id'].'"');

                        $dataset->Rows[0]["@id"] = $variations_val['@id'];

                        $dataset->Rows[0]["amazonErrorReason"] = $amazon_error_reason;

                        $dataset->Rows[0]["isVisibleOnAmazonCAN?"] = TRUE;

                        $dataset->Rows[0]["recordHistory"] = $record_history;

                        $dataset->Rows[0]["isReadyForUpdateToCanAmz?"] = FALSE;
...

        {
...

            {
                ...
                $query = $mysqli->query('UPDATE ready_for_amazon SET ASIN = "'.$asin.'", `recordHistory` = "'.$record_history.'", `amazonErrorReason` = "'.$amazon_error_reason.'", `isVisibleOnAmazonCAN?` = "1" WHERE `@id` = "'.$val['@id'].'"');                
