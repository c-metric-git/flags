...
$mysqli->query('DELETE FROM ready_for_amazon WHERE `Product - isAmazonCANProduct?` != "1" AND `Product - isAmazonUKProduct?` != "1"');

$mysqli->query('DELETE FROM ready_for_amazon_child WHERE `Product - isAmazonCANProduct?` != "1" AND `Product - isAmazonUKProduct?` != "1"');
...
$result = $api->Query("SELECT [platinum-5],[platinum-4],[platinum-3],[platinum-2],[platinum-1],
                       ...
                       FROM [Amazon Entry] WHERE [isReadyForUpdate?] AND [Product - isFlagsrusProduct?]<>true ORDER BY [sku] DESC");     //[priceAmazon],[shippingCost],[shippingCostNew]
