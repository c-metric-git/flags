...
$sql = 'SELECT `@id`,`sku`,`recordHistory`,`amazonErrorReason` FROM ready_for_amazon WHERE `Product - isAmazonUKProduct?` = "1"';
...
$dataset = $api->GetSchema("Amazon Entry",array('isVisibleOnAmazonUK?','isReadyForUpdateToUkAmz?','recordHistory','amazonErrorReason','UK Amz ASIN','Amazon Title','Amazon_Style_Number'));
...

    {
            ...
            {            
                ...
                {                
...

                    {
                        ...
                        $query = $mysqli->query('UPDATE ready_for_amazon_child SET `UK Amz ASIN` = "'.$asin.'", `recordHistory` = "'.$record_history.'", `amazonErrorReason` = "'.$amazon_error_reason.'", `isVisibleOnAmazonUK?` = "1" WHERE `@id` = "'.$variations_val['@id'].'"');

                        $dataset->Rows[0]["@id"] = $variations_val['@id'];

                        $dataset->Rows[0]["amazonErrorReason"] = $amazon_error_reason;

                        $dataset->Rows[0]["isVisibleOnAmazonUK?"] = TRUE;

                        $dataset->Rows[0]["recordHistory"] = $record_history;

                        $dataset->Rows[0]["isReadyForUpdateToUkAmz?"] = FALSE;
...

        {
...

            {
                ...
                $query = $mysqli->query('UPDATE ready_for_amazon SET UK Amz ASIN = "'.$asin.'", `recordHistory` = "'.$record_history.'", `amazonErrorReason` = "'.$amazon_error_reason.'", `isVisibleOnAmazonUK?` = "1" WHERE `@id` = "'.$val['@id'].'"');                
                ...
                $dataset->Rows[0]["UK Amz ASIN"] = $asin;
