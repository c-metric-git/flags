...
$submissionProduct_list = $mysqli->query('SELECT product_id, sku FROM error_reporting WHERE processed="0" AND (feed_type = "Product Posting" OR feed_type = "Variation Posting")  AND `isAmazonCANProduct?` = "1" GROUP BY sku');
$TDProductId = array();
...
$submission_list = $mysqli->query('SELECT id, product_id, sku, feed_type, feed_id FROM error_reporting WHERE processed="0" AND feed_type = "Product Posting"  AND `isAmazonCANProduct?` = "1" GROUP BY feed_id');
...
$dataset = $api->GetSchema("Amazon Entry",array('recordHistory','isReadyForUpdateToCanAmz?','isReadyForDeletionToCanAmz?','amazonErrorReason','isVisibleOnAmazonCAN?'));
...

{
    ...
    {        
        ...
        $dataset->Rows[0]["isVisibleOnAmazonCAN?"] = 0;

        //$dataset->Rows[0]["ASIN"] = '';

        $dataset->Rows[0]["isReadyForUpdateToCanAmz?"] = 0;

        $dataset->Rows[0]["isReadyForDeletionToCanAmz?"] = 0;
