...
{    
    ...
    $feed_arr[$key]['product-parent']   = (isset($feeds['Amazon category - item-type']) && $feeds['Amazon category - item-type'] != '') ? $feeds['Amazon category - item-type'] : ((isset($feeds['product_type']) && $feeds['product_type'] != '') ? $feeds['product_type'] : 'Accessory');    
    ...
    if($feed_arr[$key]['parentage'] == 'parent')
    {
        ...
        {            
            ...
            $feed_arr[$key]['variations'][$count]['product-parent']                = (isset($feeds['Amazon category - item-type']) && $feeds['Amazon category - item-type'] != '') ? $feeds['Amazon category - item-type'] : ((isset($feeds['product_type']) && $feeds['product_type'] != '') ? $feeds['product_type'] : 'Accessory');
...
foreach($feed_arr as $key=>$product_feed)
{
    ...
    $cwd = getcwd(); 
    $tmpfname = $cwd."/product_xml_files/product_update_".date('Y').date('m').date('d')."_".date('H').date("i").".xml";  
    ...
    if(isset($product_feed['variations']) && count($product_feed['variations']))
    {
        ...
        $marketplaceIdArray = array("Id" => array(MARKETPLACE_ID));
        $tmpfname = $cwd."/product_varaiation_xml_files/product_update_".date('Y').date('m').date('d')."_".date('H').date("i").".xml";  
        //$tmpfname = tempnam("/tmp", "FOO");
        $feedHandle = fopen($tmpfname, 'w+');
        fwrite($feedHandle, $feed);
        rewind($feedHandle);
...
function invokeSubmitFeed_amzn(MarketplaceWebService_Interface $service, $request, $product_feed)
{
    ...
    {
        $mysqli = new mysqli(DB_HOST, DB_USER, DB_PASSWORD, DB_NAME);        
        $mysqli->query('INSERT INTO error_reporting VALUES (null, "'.$product_feed['product_id'].'", "'.$product_feed['SKU'].'","Product Posting",null,"503 error on product feed call","0",null)');        
...
function invokeSubmitFeed_image(MarketplaceWebService_Interface $service, $request, $image,$product_feed)
{
    ...
    {
        $mysqli = new mysqli(DB_HOST, DB_USER, DB_PASSWORD, DB_NAME);        
        $mysqli->query('INSERT INTO error_reporting VALUES (null,"'.$product_feed['product_id'].'", "'.$product_feed['SKU'].'","Image Posting",null,"503 error for image posting","0",null)');
...
function invokeSubmitFeed_amzn_relations(MarketplaceWebService_Interface $service, $request,$product_feed)
{
    try
    {
        ...
        {                
            $submitFeedResult = $response->getSubmitFeedResult();
            $mysqli = new mysqli(DB_HOST, DB_USER, DB_PASSWORD, DB_NAME);
            $mysqli->query('INSERT INTO error_reporting VALUES (null, "'.$product_feed['product_id'].'", "'.$product_feed['SKU'].'","Variation Posting","'.$response->SubmitFeedResult->FeedSubmissionInfo->FeedSubmissionId.'",null,"0",null)');
            if ($submitFeedResult->isSetFeedSubmissionInfo()) 
            {                    
                $feedSubmissionInfo = $submitFeedResult->getFeedSubmissionInfo();
                if ($feedSubmissionInfo->isSetFeedSubmissionId()) {
    ...
    catch (MarketplaceWebService_Exception $ex) 
    {
        echo("Caught Exception: " . $ex->getMessage() . "\n");
        $mysqli = new mysqli(DB_HOST, DB_USER, DB_PASSWORD, DB_NAME);
        $mysqli->query('INSERT INTO error_reporting VALUES (null, "'.$product_feed['product_id'].'", "'.$product_feed['SKU'].'","Variation Posting",null,"503 error for variation posting","0",null)');        
        sleep(190);
        invokeSubmitFeed_amzn_relations($service, $request,$product_feed);
    }
}
...
function invokeSubmitFeed_qnt(MarketplaceWebService_Interface $service, $request, $qunt, $product_feed)
{
    ...
    {
        $mysqli = new mysqli(DB_HOST, DB_USER, DB_PASSWORD, DB_NAME);        
        $mysqli->query('INSERT INTO error_reporting VALUES (null, "'.$product_feed['product_id'].'", "'.$product_feed['SKU'].'","Quantity Posting",null,"503 error for quantity posting","0",null)');
        echo json_encode(array("Caught_Exception" => $ex->getMessage(), "Response_Status_Code" => $ex->getStatusCode(), "Error_Code" => $ex->getErrorCode(), "Error_Type" => $ex->getErrorType(), "Request_ID" => $ex->getRequestId(), "XML" => $ex->getXML(), "ResponseHeaderMetadata" => $ex->getResponseHeaderMetadata()));        
        sleep(120);
        invokeSubmitFeed_qnt($service, $request, $qunt, $product_feed);
    }
...
function invokeSubmitFeed_price(MarketplaceWebService_Interface $service, $request, $price, $product_feed)
{
    ...
    {
        $mysqli = new mysqli(DB_HOST, DB_USER, DB_PASSWORD, DB_NAME);        
        $mysqli->query('INSERT INTO error_reporting VALUES (null,"'.$product_feed['product_id'].'", "'.$product_feed['SKU'].'","Price Posting",null,"503 error for price posting","0",null)');
        echo json_encode(array("Caught_Exception" => $ex->getMessage(), "Response_Status_Code" => $ex->getStatusCode(), "Error_Code" => $ex->getErrorCode(), "Error_Type" => $ex->getErrorType(), "Request_ID" => $ex->getRequestId(), "XML" => $ex->getXML(), "ResponseHeaderMetadata" => $ex->getResponseHeaderMetadata()));
        sleep(190);
