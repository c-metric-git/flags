...
(function($){
	...
	bValidator = function(mainElement, overrideOptions, instanceName){
		...
		var options = {
			...
			errorMessages: {
				en: {
					...
					'alphanum1':   'Please enter both alphanumeric characters .',
					'digit':      'Please enter only digits.',
					'number':     'Please enter a valid number.',
					'number1':     'Please enter a valid number.',
					'email':      'Please enter a valid email address.',
					'mobile':      'Please enter a valid contact Number.',
					'image':      'This field should only contain image types',
					'url':        'Please enter a valid URL.',
                                        'url1':       'Please enter a valid URL.',
		...
		validator = {
			...
			alphanum1: function(v){
				return this.regex(v, /^[A-Za-z][A-Za-z0-9.]*(?:_[A-Za-z0-9]+)*$/);
			},
			...
			number1: function(v){
				return this.regex(v, /^[1-9][0-9]{0,10}$/);
			},
			...
			url: function(v){
				return this.regex(v, /^(http|https|ftp)\:\/\/[a-z\d\-\.]+\.[a-z]{2,3}(:[a-z\d]*)?\/?([a-z\d\-\._\?\,\'\/\\\+&amp;%\$#\=~])*$/i);
                        ...
                        url1: function(v){
                                return this.regex(v, /^((http|https|ftp)\:\/\/)?\s*www\.[a-z\d\-\.]+\.[a-z]{2,3}(:[a-z\d]*)?\/?([a-z\d\-\._\?\,\'\/\\\+&amp;%\$#\=~])*$/i);
				//return this.regex(v, /^((ftp|http|https):\/\/)?\s*www\.[a-z\d\-]{1,255}\.[a-z]{2,6}\s*$/);
			},
