...
//echo $db['default']['database'];
	...
	//$BASEDIR = 'c:/dev/src/php/amzordermails/amzordermails';
		...
		define('AMAZON_MWS_MERCHANT_ID', 'A18YWFZ6SIWB7S');
		define('AMAZON_MWS_MARKETPLACE_ID', 'ATVPDKIKX0DER');
		define('AMAZON_MWS_ACCESS_KEY', 'AKIAJ2XX4VAO7BQPZ3XQ');
		define('AMAZON_MWS_SECRET_KEY', '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');
		...
		// Sears Cridencials
		define('Sears_Email', "virendra_chunara@tops-int.com");
	...
	public function insertUnShippedOrders($orders, $orderItems)
    {
		...
		$order->purchaseDate = str_ireplace('Z', '', $order->purchaseDate);
                ...
                foreach($orderItems as $orderItem) {
                    ...
                    $values .= "('{$orderItem->orderId}','{$orderItem->OrderStatus}', '{$orderItem->orderItemId}', '".userid."', '{$order->orderTotalAmount}', 'amzn', '{$order->purchaseDate}', '{$orderItem->sku}', '{$orderItem->title}', '{$order->shipToName}', '{$order->shipmentServiceLevelCategory}', '{$order->buyerEmail}', '$orderItem->quantityOrdered', '{$order->shipToAddressLine1}', '{$order->shipToAddressLine2}', '{$order->shipToCity}', '{$order->shipToStateOrRegion}', '{$order->shipToPostalCode}' )";
            ...
            $sql = "INSERT IGNORE INTO amazon_order_items VALUES " . $values."			
					ON DUPLICATE KEY UPDATE OrderStatus='{$orderItem->OrderStatus}', order_total_amount='{$order->orderTotalAmount}', type='amzn', purchase_date='{$order->purchaseDate}', sku='{$orderItem->sku}', title='{$orderItem->title}', ship_to_name='{$order->shipToName}', ship_service_level='{$order->shipmentServiceLevelCategory}', buyer_email='{$order->buyerEmail}' , Qty='{$orderItem->quantityOrdered}', ship_address_1='{$order->shipToAddressLine1}', ship_address_2='{$order->shipToAddressLine2}', ship_city='{$order->shipToCity}', ship_state='{$order->shipToStateOrRegion}', ship_postal_code='{$order->shipToPostalCode}'"; // purchase date in utc
			//echo $sql;exit;
    ...
    public static function saveCsv()
    {
        ...
        try 
        {
            ...
            if(count($orders['orders'])>0)
            {
                ...
                $message = array('msg'=>"<b>Order synchronized successfully</b>",'status'=>0);
            }
            else
                $message = array('msg'=>"<b>No Orders Found</b>",'status'=>0);
...
if ($_REQUEST['argc'] > 1) 
{
    ...
    if ($_REQUEST['argc'] == 2 && $_REQUEST['argv'] == 'save-csv')
        AllOrders::saveCsv();
}
