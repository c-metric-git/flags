...
class ModelSaleOrder extends Model {
	...
	public function getSuppliers() {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "supplier ORDER BY sort_order, name ASC");
		
		return $query->rows;
	}
	public function getOrdersProductsSum($data = array()) {
	
		$sql = "SELECT DISTINCT `product_id`, name, sum( quantity ) AS quantity FROM `" . DB_PREFIX . "order_product` WHERE `status_id` = '1'";
		
		if (isset($data['filter_name']) && !is_null($data['filter_name'])) {
			$sql .= " AND LOWER(name) LIKE '%" . strtolower($this->db->escape($data['filter_name'])) . "%'";
		}

		if (isset($data['supplier_id']) && !is_null($data['supplier_id'])) {
			$sql .= " AND supplier_id = '" . (int)$data['supplier_id'] . "'";
		}
