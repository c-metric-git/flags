            ...
            if (is_array($fieldType)) {
                if ($this->isComplexType($fieldType[0])) {
                    ...
                    if ($elements->length >= 1) {
                        require_once ($_SERVER['DOCUMENT_ROOT'].preg_replace('@/+$@','',dirname($_SERVER['SCRIPT_NAME'])).'/API/amzn/Reports/'.str_replace('_', '/', $fieldType[0]).".php");
                ...
                if ($this->isComplexType($fieldType)) {
                    $elements = $xpath->query("./a:$fieldName", $dom);
                    if ($elements->length == 1) {
                        require_once ($_SERVER['DOCUMENT_ROOT'].preg_replace('@/+$@','',dirname($_SERVER['SCRIPT_NAME'])).'/API/amzn/Reports/'.str_replace('_', '/', $fieldType). ".php");
