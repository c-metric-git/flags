...
class TeamDeskWebprofiles {
        ...
        */
        ...
        if($this->api !='' )
        {
            ...
            */
            ...
            print_R($this->ConfigAttributeArray);   
            exit; */   
            /**
            ...
            */
            if (isset($arrTDProductsAttributes) && is_array($arrTDProductsAttributes) && count($arrTDProductsAttributes) > 0) {
                       foreach ($arrTDProductsAttributes as $key => $tdProductAtt) {
                              ...
                              if(!in_array($tdProductAtt['PinnacleAttributeSku'],$product_added_sku) && ($tdProductAtt["Product - Weight"]>0)) {
                                  ...
                                  for($i=0;$i<$categories_counter;$i++) { 
                                      ...
                                      if($i == 0) { 
                                            ...
                                            $csv_row[] = "base";  //_product_websites
                                            $csv_row[] = ""; //bulky
                                            $csv_row[] = $tdProductAtt['Color'];//$tdProductAtt['Web Option Label']=="Color"?$tdProductAtt["Attribute"]:""; //color 
                                            ...
                                            $csv_row[] = $tdProductAtt['Length'];//$tdProductAtt['Web Option Label']=="Length"?$tdProductAtt["Attribute"]:"";  //length
                                            ...
                                            $csv_row[] = $tdProductAtt['Size'];//$tdProductAtt['Web Option Label']==""?$tdProductAtt["Attribute"]:""; //size 
                                            $csv_row[] = $tdProductAtt['is_active']=="Yes"?1:2;  //status
                                            $csv_row[] = $tdProductAtt['Style'];//$tdProductAtt['Web Option Label']=="Style"?$tdProductAtt["Attribute"]:""; //style 
                                            $csv_row[] = "2";  //tax_class_id
                                            $csv_row[] = ""; //url_path
                                            $csv_row[] = "4"; //visibility
            ...
            if (isset($arrTDProducts) && is_array($arrTDProducts) && count($arrTDProducts) > 0) {
                   foreach ($arrTDProducts as $tdProduct) {  
                       ...
                       if(!in_array($tdProduct['PinnacleSKU'],$product_added_sku)  && ($tdProduct["weight_calced"]>0)) { 
                            ...
                            if(count($this->ConfigAttributeArray[$lowerpinnacleSKU]) > 0) {
                                 //$ConfigurableProductArray[] = $tdProduct;
                                 $attribute_set = $this->ConfigAttributeArray[$lowerpinnacleSKU][0]['Web Option Label']; 
                            ...
                            $categories_counter = count($this->arrTDProductCategories[$lowerpinnacleSKU]);
                            $attributes_counter = count($this->ConfigAttributeArray[$lowerpinnacleSKU]);
                            ...
                            for($i=0;$i<$largest_counter;$i++) {
                                    $csv_row = array();
                                        if($i == 0) {
                                            ...
                                            $csv_row[] = ""; //_media_position
                                            $csv_row[] = ""; //_media_is_disabled
                                            $csv_row[] = $this->ConfigAttributeArray[$lowerpinnacleSKU][$i]['PinnacleAttributeSku'];; //$tdProduct["PinnacleAttributeSku"];  //_super_products_sku
                                            $csv_row[] = strtolower($this->ConfigAttributeArray[$lowerpinnacleSKU][$i]['Web Option Label']);  //_super_attribute_code
                                            $csv_row[] = $this->ConfigAttributeArray[$lowerpinnacleSKU][$i]['Attribute']; //$tdProduct["Attribute"]; //_super_attribute_option
                                            $price = 0;    
                                            $tmpProductPrice = 0;
                                            $attributePrice = 0; 
                                            ...
                                            if ($this->ConfigAttributeArray[$lowerpinnacleSKU][$i]['DiscountPriceCalced'] > 0) {
                                                $attributePrice =  $this->ConfigAttributeArray[$lowerpinnacleSKU][$i]['DiscountPriceCalced'];
                                            }
                                            else {
                                                $attributePrice =  $this->ConfigAttributeArray[$lowerpinnacleSKU][$i]['PriceCalced'];
                                            }       
                                            ...
                                            $csv_row[] = $price;//$arrTDProductsAttributes[$lowerpinnacleSKU][$i]['DiscountPriceCalced']>0?$arrTDProductsAttributes[$lowerpinnacleSKU][$i]['DiscountPriceCalced']:"";//$tdProduct["Product - Quantity Available"];  //_super_attribute_price_corr
                                                ...
                                                $csv_row[] = ""; //_media_is_disabled
                                                $csv_row[] = $this->ConfigAttributeArray[$lowerpinnacleSKU][$i]['PinnacleAttributeSku'];; //$tdProduct["PinnacleAttributeSku"];  //_super_products_sku
                                                $csv_row[] = strtolower($this->ConfigAttributeArray[$lowerpinnacleSKU][$i]['Web Option Label']);
                                                $csv_row[] = $this->ConfigAttributeArray[$lowerpinnacleSKU][$i]['Attribute'];; //$tdProduct["Attribute"]; //_super_attribute_option
                                                $price = 0;    
                                                $tmpProductPrice = 0;
                                                $attributePrice = 0; 
    ...
    */
    private function getTDProducts()
    {
        ...
        */       
        $strColumns = "[PinnacleSKU],[Solo Product Description],[Product - imgLocation350],[Display Name],[PriceCalced],[DiscountPriceCalced],[overview],[is_visible],[weight_calced],[Quantity Available],[imgLocationCustom],[Related Product]";   
       try
       {        
    ...
    */ 
    private function getTDProductsAttributes()    
    {
       ...
       try
       {
            $arrResults = $this->api->Query("SELECT ".$strColumns." FROM [SS Attribute] ".$arrQueries ." ORDER BY [PinnacleAttributeSku],[Web Option Label] ASC");  
            /*echo '<pre>'; 
            print_R($arrResults->Rows);    */
            if (isset($arrResults->Rows)) {    
                foreach ($arrResults->Rows as $tdProductAtt) {    
                     $att_counter=0;
                     $PinnacleAttributeSku = (string) $tdProductAtt['PinnacleAttributeSku'];
                     $PinnacleAttributeSku = strtolower($PinnacleAttributeSku);
                     if(!isset($arrProductAttributes[$PinnacleAttributeSku])) {
                        $arrProductAttributes[$PinnacleAttributeSku] = $tdProductAtt;
                     }
                     $label = $tdProductAtt['Web Option Label'];
                     $arrProductAttributes[$PinnacleAttributeSku][$label] = $tdProductAtt['Attribute'];
                     $pos = strpos($arrProductAttributes[$PinnacleAttributeSku]['Web Option Label'],$tdProductAtt['Web Option Label']);
                     if((isset($arrProductAttributes[$PinnacleAttributeSku]['Web Option Label'])) && ($pos===false)) {
                         $arrProductAttributes[$PinnacleAttributeSku]['Web Option Label'] = $arrProductAttributes[$PinnacleAttributeSku]['Web Option Label']."/".$tdProductAtt['Web Option Label'];
                     }
                     else {
                         $arrProductAttributes[$PinnacleAttributeSku]['Web Option Label'] = $tdProductAtt['Web Option Label'];   
                     } 
                     $productSKU = (string) $tdProductAtt['Related Web Profile'];
                     $productSKU = strtolower($productSKU);
                     $this->ConfigAttributeArray[$productSKU][] = $tdProductAtt;
            ...
            //print_R($arrResults->Rows);
            /*echo '<pre>';
            print_R($arrProductAttributes);
            echo count($arrProductAttributes);
            exit;*/  
    ...
    */
    public function isProductEventExist($sku){
        $arrResults = $this->api->Query('SELECT [Related Product] FROM [Product Events] WHERE [Label]="Website Start Record" AND [Related Website]="FL" AND [isNewProduct?]=true AND [Related Product]="'.$sku.'"');
