...
var CompareNoReload = {
    ...
    { jQuery('#resultLoading').remove();
        ...
        new Ajax.Request(removeUrl, {
                ...
                var resultData = jQuery('#itemcount').html();

                if(jQuery('#mini-compare .count').length==0){
                    jQuery( '<span class="count"></span>' ).insertAfter( ".mini-compare-heading .icon" );
                }

                jQuery('#mini-compare .count').html(resultData);
                    hideLoadingImage();
            }
        });
    },
    ...
    {
        jQuery('#resultLoading').remove();
        showLoadingImage('Removing compare data... please wait..');
        new Ajax.Request(clearUrl, {
                ...
                jQuery('div#header-compare').empty();  // by brt
                $$('div#header-compare')[0].insert({bottom:'<div class="block-compare"></div>'}); // by brt
                CompareNoReload.compareUpdateSidebarHtml(response.responseText);
                CompareNoReload.rescanForNewCompare();
                jQuery('#mini-compare .count').remove();
                hideLoadingImage();
                }
    ...
    {
        jQuery('#resultLoading').remove();
        showLoadingImage('Adding data to compare .. please wait..');
        jQuery('div#header-compare').empty();
        $$('div#header-compare')[0].insert({bottom:'<div class="block-compare"></div>'});
...
if(!jQuery('#compare-items').length>0) {
    jQuery('div#header-compare').empty();
    $$('div#header-compare')[0].insert({bottom:'<div class="block block-list block-compare"> <div id="itemcount" style="display: none;">0</div> <div class="block-content"> <p class="empty">You have no items to compare.</p> </div> </div>    </div>'});

}else{
    $$('div#header-compare')[0].insert({bottom:'<div class="block-compare"></div>'});
}

//if($$('div#header-compare').length>0)$$('div#header-compare')[0].insert({bottom:'<div class="block-compare"></div>'});
...
//=============== on click event by brt

//link-compare

function showLoadingImage(text){

    jQuery('body').append('<div id="resultLoading" style="display:none"><div><img src="http://w3lessons.info/demo/ajax-indicator/ajax-loader.gif"><div>'+text+'</div></div><div class="bg"></div></div>');
