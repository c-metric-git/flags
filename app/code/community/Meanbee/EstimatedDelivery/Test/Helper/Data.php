...
class Meanbee_EstimatedDelivery_Test_Helper_Data extends EcomDev_PHPUnit_Test_Case {
     ...
     * @loadFixture estimatedDelivery.yaml
     */
    public function testGetDaysUntilDispatchDate($testId, $startDate, $shippingMethod) {
        $expectation = $this->expected($testId);

        $date = new Zend_Date($startDate);
        $expectedDays = $expectation->getResult();
        $latestDispatchTimePredicate = $expectation->getLastDispatchTime();
        $dispatchPreparationPredicate = $expectation->getDispatchPreparation();
        $dispatchableDaysPredicate = $expectation->getDispatchableDays();
     ...
     * @loadFixture estimatedDelivery.yaml
     */
    public function testDaysGetRoundedCorrectly() {
        // I noticed an issue that since the comparison between the two days is accurate, the
        // page load time can factor in and you'll get something like:
        // "Your order will arrive in 5-10.9999998 days" this tests this doesn't happen.
        $startDateFrom = new Zend_Date('3rd March 2014 14:00');
        $startDateTo = new Zend_Date('3rd March 2014 14:01');
        $shippingMethod = 'test_shipping1';

        $from = $this->_helper->getDaysUntilEstimatedDeliveryFrom($shippingMethod, $startDateFrom);
        $to = $this->_helper->getDaysUntilEstimatedDeliveryTo($shippingMethod, $startDateTo);

        $this->assertEquals(3, $from);
        $this->assertEquals(5, $to);
