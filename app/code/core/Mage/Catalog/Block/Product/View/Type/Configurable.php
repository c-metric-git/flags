...
class Mage_Catalog_Block_Product_View_Type_Configurable extends Mage_Catalog_Block_Product_View_Abstract
{
	...
	public function getJsonConfigsp()
    {
/* 		$spData = array();
...
/*             $spData[$_sp->getId()] = 
                (string)$this->helper('catalog/image')
                    ...
                    ->resize(645,520);            
			$spData[$_sp->getId()]['sku'] = (string)$_sp->getSku();
			$spData[$_sp->getId()]['qty'] = (string)$_sp->getQty();
			$spData[$_sp->getId()]['parentid'] = (string)$_sp->getParentId();
			//$spData[$_sp->getId()][] = (string)$_sp->getQty();
					
        }
        return Mage::helper('core')->jsonEncode($spData); */
		        $attributes = array();
        $options    = array();
        $store      = $this->getCurrentStore();
        $taxHelper  = Mage::helper('tax');
		...
		$options['parent_regular_price'] = $currentProduct->getPrice();
		$options['parent_special_price'] = $currentProduct->getFinalPrice();
		$options['parent_id'] = $currentProduct->getId();
        ...
        foreach ($this->getAllowProducts() as $product) {
            ...
            if (is_array($prices)) {
                foreach ($prices as $value) {
					if($value['value_index'] == $attributeValue && $value['pricing_value'])
					{	
                    $configurablePrice = $currentProduct->getConfigurablePrice();

					$options[$attributeValue]['special_price'] = $configurablePrice;
					$options[$attributeValue]['regular_price'] = $this->_prepareOldPrice($value['pricing_value'], $value['is_percent']);
/* 					echo "<pre>";
					print_r($this->_prepareOldPrice($value['pricing_value'], $value['is_percent']));
					exit; */
/*                     $info['options'][] = array(
                        'id'        => $value['value_index'],
                        'label'     => $value['label'],
				...
				$options[$attributeValue]['parent_regular_price'] = $currentProduct->getPrice();
				$options[$attributeValue]['parent_special_price'] = $currentProduct->getFinalPrice();
                $options[$attributeValue]['product_id'] = $productId;
                $options[$attributeValue]['sku'] = $product->getSku();
