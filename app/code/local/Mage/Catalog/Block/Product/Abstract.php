...
abstract class Mage_Catalog_Block_Product_Abstract extends Mage_Core_Block_Template
{
    ...
    /* jp */
    protected $_priceBlockDefaultTemplate_pd = 'catalog/product/product_detail_price.phtml';
    ...
    protected function _getPriceBlockTemplate_pd($productTypeId)
    {
        if (isset($this->_priceBlockTypes[$productTypeId])) {
            if ($this->_priceBlockTypes[$productTypeId]['template'] != '') {
                return $this->_priceBlockTypes[$productTypeId]['template'];
        ...
        return $this->_priceBlockDefaultTemplate_pd;
    }
    ...
    public function _preparePriceRenderer_dp($productType)
    {
        return $this->_getPriceBlock($productType)
            ->setTemplate($this->_getPriceBlockTemplate_pd($productType))
            ->setUseLinkForAsLowAs($this->_useLinkForAsLowAs);
    }
    ...
    /* jp */
    public function getPriceHtml_pd($product, $displayMinimalPrice = false, $idSuffix = '')
    {
        $type_id = $product->getTypeId();
        if (Mage::helper('catalog')->canApplyMsrp($product)) {
            $realPriceHtml = $this->_preparePriceRenderer_dp($type_id)
                ->setProduct($product)
                ->setDisplayMinimalPrice($displayMinimalPrice)
                ->setIdSuffix($idSuffix)
                ->toHtml();
        ...
        return $this->_preparePriceRenderer_dp($type_id)
            ->setProduct($product)
            ->setDisplayMinimalPrice($displayMinimalPrice)
            ->setIdSuffix($idSuffix)
            ->toHtml();
