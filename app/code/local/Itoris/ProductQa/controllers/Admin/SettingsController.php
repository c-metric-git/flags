...
class Itoris_ProductQa_Admin_SettingsController extends Itoris_ProductQa_Controller_Admin_Controller {
	...
	public function indexAction() {
		...
		if (!empty($websiteCode)) {
			...
			if (!Mage::helper('itoris_productqa')->isRegistered($website)) {
		...
		$this->loadLayout();
		$settings = $this->getLayout()->createBlock('itoris_productqa/admin_config_settings');
	...
	public function saveAction() {
		...
		$model = Mage::getModel('itoris_productqa/settings');
		$validation = $model->_isValid($settings);
	...
	protected function _isAllowed() {
		return Mage::getSingleton('admin/session')->isAllowed('admin/system/itoris/itoris_productqa');
	}
}
?>